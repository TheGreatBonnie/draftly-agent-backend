...
GROUNDEDNESS_RUBRIC = (
    ...
    "supplied for issue/support/release runs — without inventing APIs, endpoints, or "
    "behavior that are not supported. Treat every API, method, parameter, and behavior "
    "named in the documentation as traceable if it appears anywhere in the provided "
    "evidence content, diff, changed files, or referenced source. Any claim must be "
    "traceable to the evidence, the change being documented, the actual diff, or the "
    "provided evidence/content files. Do not require a diff when the run is an "
    "issue/support/release surface with no diff. A run whose <ActualEnvironmentState> "
    "gate reports result_status INTERRUPTED with deliver_ran False was paused by the "
    "ReviewGate for human review before final delivery — the authored documentation "
    "under evaluation is still complete, so treat it as a normal state and do not "
    "penalize groundedness for the interrupt or the absent delivery step. Score 0-1 "
