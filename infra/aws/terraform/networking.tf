...
resource "aws_subnet" "db" {
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-db-${data.aws_availability_zones.available.names[count.index]}"
    Type = "database"
  }
}
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-private-rt-${data.aws_availability_zones.available.names[count.index]}"
  }
}
